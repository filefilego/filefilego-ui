    ...
    <div id="node-container"
            ...
            <div class="uk-text-center">
                ...
                <div v-else>
                    ...
                    <div v-else style="text-align:left; text-transform: none;">
                        ...
                        <div v-else>
                            <!-- entry  -->
                            <div v-if="node.node_type == 3 || node.node_type == 4"
                                style="padding-left: 15px; padding-bottom: 15px; padding-right: 15px;">
                                <div v-if="node.node_type == 3 && hasMedia(node)" style="text-align: center;">
                                    <div
                                        style="padding-bottom:7px; display: flex; justify-content: center; align-items: center;">
                                        <div style="padding:0px;" v-if="loadingMedia">
                                            loading media
                                            <span style="color:#3e15ca;" class="uk-margin-small-right"
                                                uk-spinner="ratio: 1"></span>
                                        </div>
                                        <div v-else style="padding:0px;">
                                            <div v-if="getCoverImageURL() != ''" style="padding:0px;" uk-lightbox>
                                                <a data-type="image" :href="getCoverImageURL()">
                                ...
                                <div v-if="getListImagesURL().length > 0">
                                    <div style="text-align: center;">
                                        <h2 style="margin-bottom: 10px;" class="modal-header">Gallery</h2>
                                    </div>
                                    <div style="display: flex; justify-content: center; align-items: center;"
                                        class="uk-text-center" uk-lightbox uk-grid>
                                        <div v-for="m in getListImagesURL()" :key="m">
                                            <div style="padding:0px;" class="uk-card uk-card-default uk-card-body">
                                                <a data-type="image" :href="m">
                                                    <img style="max-height: 120px; max-width: 120px;" :src="m" />
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div style="margin-top:15px;" v-if="node.nodes.length == 0 && node.node_type == 4">
                                    <div style="text-align: center;"><svg xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" width="80pt" height="80pt"
                                            ...
                                            <defs>
                                                ...
                                                <symbol id="r" overflow="visible">
                                                    <path d="m2.4531-22.156h5.0938v22.156h-5.0938z"></path>
                                                </symbol>
                                                <symbol id="c" overflow="visible">
                                                    <path
                                                        d="m10.047-12.688c-1.1367 0-2 0.40625-2.5938 1.2188s-0.89062 1.9805-0.89062 3.5c0 1.5312 0.29688 2.7031 0.89062 3.5156s1.457 1.2188 2.5938 1.2188c1.1016 0 1.9453-0.40625 2.5312-1.2188 0.59375-0.8125 0.89062-1.9844 0.89062-3.5156 0-1.5195-0.29688-2.6875-0.89062-3.5-0.58594-0.8125-1.4297-1.2188-2.5312-1.2188zm0-3.6562c2.7383 0 4.8789 0.74609 6.4219 2.2344 1.5391 1.4805 2.3125 3.5273 2.3125 6.1406 0 2.625-0.77344 4.6797-2.3125 6.1562-1.543 1.4805-3.6836 2.2188-6.4219 2.2188-2.7617 0-4.918-0.73828-6.4688-2.2188-1.5547-1.4766-2.3281-3.5312-2.3281-6.1562 0-2.6133 0.77344-4.6602 2.3281-6.1406 1.5508-1.4883 3.707-2.2344 6.4688-2.2344z">
                                        ...
                                        <div>
                                            <span style="color: rgb(0, 0, 0); font-weight: bold;"> This folder is empty
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div style="text-align: center;">
                                    ...
                                    <div style="margin-top:10px; text-align:left;">
                                        ...
                                        <br /> <span class="normal-txt" style="font-weight:500;"> Created At: </span><span
                                            class="normal-txt"> {{ $filters.timestamptodate(node.timestamp) }} </span>
        ...
        <div id="modal-newfolder" uk-modal="container:#node-container">
            <div class="uk-modal-dialog uk-modal-body">
                ...
                <div style="padding: 10px; margin-top:5px;">
                    <button v-if="!creatingFolder && !folderCreated" @click="createNewFolder" class="uk-button ffg-button"
        ...
        <div id="modal-upload" uk-modal="container:#node-container; esc-close:false; bg-close:false; ">
            <div style="padding-top:17px; width:60%; padding-bottom: 20px;" class="uk-modal-dialog uk-modal-body">
                ...
                <div style="padding: 10px; margin-top:10px;">
                    <uploader-component :maxfiles="0" :maxsize="maxFileSizeChannel" :uploadmedia="uploadIsForMedia"
                        :callback="reload" :parent="node.node_hash" place="channel" />
                </div>
            </div>
        </div>

        <div id="modal-upload-media" uk-modal="container:#node-container; esc-close:false; bg-close:false; ">
            <div style="padding-top:17px; width:60%; padding-bottom: 20px;" class="uk-modal-dialog uk-modal-body">
                <button id="close-modal-create" class="uk-modal-close-default" type="button" uk-close></button>
                <h2 class="modal-header">Upload media files</h2>
                <div style="padding: 10px; margin-top:10px;">
                    <uploader-component :maxfiles="5" :maxsize="maxFileSizeMedia" :uploadmedia="uploadIsForMedia"
                        :callback="handleUploadedMediaFiles" :parent="node.node_hash" place="channel-entry"
...
export default {
    ...
    data() {
        return {
            ...
            loadingMedia: false,
            mediaFiles: [],
            maxFileSizeChannel: 0, // no limit
            maxFileSizeMedia: 1024 * 512,
            uploadIsForMedia: false,
            createEntryError: "",
            creatingEntry: false,
    ...
    methods: {
        ...
        getListImagesURL() {
            let images = []
            if (this.downloadedMedia.length > 1) {
                for (let i = 1; i < this.downloadedMedia.length; i++) {
                    images.push(this.globalState.mediaEndpoint + "?hash=" + this.downloadedMedia[i].name + "&type=" + this.downloadedMedia[i].ext)
                }
            }
            return images
        },
        ...
        extractImageDataHash() {
            ...
            if (mediaviewerDiv) {
                ...
                imageElements.forEach((img) => {
                    const dataHash = img.getAttribute("data-hash");
                    if (dataHash) {
                        imageDataHashArray.push(dataHash);
                    }
                });
                return imageDataHashArray;
            } else {
                return [];
            }
        },
        ...
        handleUploadedMediaFiles(completedUploads) {
            completedUploads.filter((o) => {
                let dupe = this.mediaFiles.filter((j) => o.file_hash == j.file_hash).length > 0
                if (!dupe) {
                    if (this.mediaFiles.length < 5) {
                        this.mediaFiles.push(o)
                        this.getPreview(o)
                    }
                }
            })
            ...
            try {
                ...
                sortedImages.filter((o) => {
                    this.mediaFiles.filter((j) => {
                        if (o == j.file_hash) {
                            attrs.push("media:" + j.file_hash + "|" + this.getFileExtension(j.name))
                        }
                    })
                })
                ...
                const data = {
                    ...
                    params: [{ nodes: [{ attributes: attrs, parent_hash: this.node.node_hash, enabled: true, description: this.entry_content, name: this.entryName, node_type: 3, timestamp: Math.floor(Date.now() / 1000) }] }],
                    id: 1
                };
        ...
        openUploadMediaModal() {
            this.uploadIsForMedia = true;
            const myModal = document.getElementById('modal-upload-media');
            const modal = window.UIkit.modal(myModal);
            modal.show();
        },
